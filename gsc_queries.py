...
def main():
    ...
    def query(service, site_url, payload):
        try:
            ...
            for row in response['rows']:
                ...
                if all(keyword not in data['query'] for keyword in ['коммерсант', 'kommersant', 'комменсантъ', 'новости', 'новостей', 'радио','kommiersant','komersant', 'ъ','fm','газета','порно', 'курс', 'прогноз', 'новости мира', 'банкротства']):
                    results.append(data)
            ...
            print(f'GSC_queries выдала ошибку')
    ...
    df_filtered = df[['query', 'impressions']]  # Выбор нужных столбцов

    df_filtered = df_filtered.astype({'query': str, 'impressions': int})

    df_filtered.to_csv('fresh_queries.csv', index=False)
    ...
    file_path = '/seo_serv/fresh_queries.csv'
    repo.index.add([file_path])

    # Создание коммита
    repo.index.commit('Добавлен файл fresh_queries.csv')

    # Отправка изменений на удаленный репозиторий
    origin = repo.remote('origin')
    origin.push()

    print("готово, запросы за вчера выгрузились (gsc_queries)")


# настройка расписания
schedule.every().day.at("11:15", "Europe/Moscow").do(main)
